...
{var canvasXY=RG.getCanvasXY(obj.canvas);var width=tooltip.offsetWidth;var height=tooltip.offsetHeight;var radius=this.radius;var mouseXY=RG.getMouseXY(window.event);tooltip.style.left=0;tooltip.style.top=canvasXY[1]+this.coords[0][1]-(height/2)-this.radius+'px';tooltip.style.left=0;tooltip.style.top=window.event.pageY-height-5+'px';tooltip.style.overflow='';if(canvasXY[0]+mouseXY[0]-(width/2)<0){tooltip.style.left=canvasXY[0]+mouseXY[0]-(width*0.1)+'px';}else if(canvasXY[0]+mouseXY[0]+(width/2)>doc.body.offsetWidth){tooltip.style.left=canvasXY[0]+mouseXY[0]-(width*0.9)+'px';}else{tooltip.style.left=canvasXY[0]+mouseXY[0]-(width/2)+'px';}};this.highlight=this.Highlight=function(shape)
